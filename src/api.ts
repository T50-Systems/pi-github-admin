	...
	for (const check of input.checks) {
		if (check === "metadata") {
			...
			const labels = await client.request(
				`/repos/${ref.owner}/${ref.name}/labels?per_page=100`,
			);
			...
			const filtered = Array.isArray(issues)
				? issues.filter((issue: any) => !issue.pull_request)
				: [];
			...
			const tagMatched = input.releaseTag
				? Array.isArray(releases) &&
					releases.some((r: any) => r.tag_name === input.releaseTag)
				: true;
...
async function getCheckRunSummary(
	...
	const checkRunResponse = (await client.request(
	...
	const statusResponse = (await client.request(
		`/repos/${ref.owner}/${ref.name}/commits/${sha}/status`,
	)) as {
		state?: string;
	...
	const failed = [
		...failedChecks,
		...(statusResponse.state && statusResponse.state !== "success"
			? [`combined-status:${statusResponse.state}`]
			: []),
		...failedStatuses,
	];
